...
def save_reconstructions(model, batch, step):
    ...
    x = batch[:8]
    y = recon[:8]
    grid = vutils.make_grid(torch.cat([(x + 1) / 2, (y + 1) / 2], dim=0), nrow=8)
...
for epoch in range(start_epoch, epochs):
    ...
    for images in pbar:
        ...
        with autocast(device_type="cuda", enabled=False):
            lp_loss = lpips_loss(recon.float().clamp(-1, 1), images.float()).mean()

        lp_ramp = min(1.0, global_step / max(1, steps_per_epoch))
        lp_weight = LPIPS_WEIGHT_START + lp_ramp * (
            LPIPS_WEIGHT_END - LPIPS_WEIGHT_START
        )
